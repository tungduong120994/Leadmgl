...
{
    public class ImportLeadBuiness
    {
        ...
        public void BulkInsertLead_Import(DataTable dt,string duAn,string sanPham, string boundCode, string postDate)
        {
            ...
            TransferLead(storesList,duAn, sanPham,boundCode,postDate);
            ...
            var storedCheckDeDup = context.CD_DieuKienCheckDuAn.Where(T => T.IsDeleted == false).Select(x => x).OrderBy(a=>a.GiaTri).ToList();
            DeDup(storedCheckDeDup, storesList);
        ...
        public List<Lead_Import> DataLead_Import()
        {
            ...
            var storesList = context.Lead_Import.Select(x => x).Take(100).OrderByDescending(t => t.Id).ToList();
        ...
        public void DeDup(List<CD_DieuKienCheckDuAn> lstcheck,List<Lead_Import> listLeadIP)
        {
            string a = "";
            int countList = lstcheck.Count;
            int abcac = 0;
            ...
            if (lstcheck.Count != 0)
            {
                foreach (var item in lstcheck)
                {
                    ...
                    if (countList == abcac)
                    {
                        if (item.TenDieuKien != "Bound_Code")
                        {
                            ...
                            sbQueryfn.Append(" SET a.Description4 ='1', a.Status_DuplicateID = B.Duplicate ,a.Process_DuplicateID=" + item.GiaTri + " ");

                            sbQueryfn.Append(" FROM Lead A");

                            sbQueryfn.Append(" JOIN mycte B  ON A.id = B.id");

                            sbQueryfn.Append(" where a.Description4 is null and Status_DuplicateID <>1 and A.Process_DuplicateID =" + (Convert.ToInt16(item.GiaTri) - 1) + " ");
                            string a111 = sbQueryfn.ToString();
                            db.Database.ExecuteSqlCommand(sbQueryfn.ToString());
                        }
                        else
                        {
                            int giatri = Convert.ToInt16(item.GiaTri);
                            var context = new Lead_GWEntities();
                            //ds Lead trung tat cả dieu kien
                            var storesListLead = context.Leads.Where(T => T.Process_DuplicateID == giatri - 1 && T.Status_DuplicateID != 1 && T.Description4 ==null).Select(x => x).ToList();
                            foreach (var item1 in storesListLead)
                            {
                                //lấy id BC của Lead
                                var ss = context.DM_BoundCode.Where(T => T.Bound_Code == item1.Bound_Code).Select(x => x.Id).ToList();
                                int bcID = 0;
                                if (ss.Count != 0)
                                {
                                    bcID = Convert.ToInt32(ss[0]);
                                }

                                //lấy toàn bộ rule bc
                                var cachckeck = context.CD_DieuKienCheckBoundCode.Where(T => T.BoundCodeId == bcID && T.IsDeleted == false).Select(x => x).ToList();
                                string ChuoiBC = "";
                                int temp = 1;
                                if (cachckeck.Count != 0)
                                {
                                    foreach (var item2 in cachckeck)
                                    {
                                        var ConKec = context.DM_BoundCode.Where(T => T.Id == item2.BoundCodeCheckId).Select(x => x.Bound_Code).ToList();
                                        ChuoiBC = ConKec[0];
                                        // }

                                        var listLeadDup = context.Leads.Where(T => T.Phone_Number == item1.Phone_Number && T.Bound_Code == (ChuoiBC) && T.Status_DuplicateID == 1).Select(x => x).ToList();
                                        temp = temp + listLeadDup.Count;

                                    }
                                    var db = new Lead_GWEntities();
                                    StringBuilder sbQueryfn = new StringBuilder("");
                                    sbQueryfn.Append(sbQueryfn);
                                    sbQueryfn.Append("UPDATE Lead SET Description4 ='1', Status_DuplicateID = " + temp + " ,Process_DuplicateID=" + item.GiaTri + " where Id   =" + item1.Id + "");
                                ...
                                else
                                {
                                    ...
                                    StringBuilder sbQueryfn = new StringBuilder("");
                                    sbQueryfn.Append(sbQueryfn);
                                    var emIu = context.Leads.Where(T => T.Id == item1.Id).Select(x => x.Status_DuplicateID).ToList();
                                    //temp=emIu;
                                    sbQueryfn.Append("UPDATE Lead SET Description4 ='1', Status_DuplicateID = 1 ,Process_DuplicateID=" + item.GiaTri + " where Id   =" + item1.Id + "");
                        ...
                        else
                        {
                            ...
                            foreach (var item1 in storesListLead)
                            {
                                ...
                                if (cachckeck.Count != 0)
                                {
                                    ...
                                    StringBuilder sbQueryfn = new StringBuilder("");
                                    sbQueryfn.Append(sbQueryfn);
                                    sbQueryfn.Append("UPDATE Lead SET Status_DuplicateID = " + temp + " ,Process_DuplicateID=" + item.GiaTri + " where Description4 is null and Id   =" + item1.Id + "");
                                ...
                                else
                                {
                                    ...
                                    var emIu = context.Leads.Where(T => T.Id == item1.Id).Select(x => x.Status_DuplicateID).ToList();
                                    //temp=emIu;
                                    sbQueryfn.Append("UPDATE Lead SET Status_DuplicateID = 1 ,Process_DuplicateID=" + item.GiaTri + " where Description4 is null and Id   =" + item1.Id + "");
            ...
            else
            {
                ...
                StringBuilder sbQueryfn = new StringBuilder("");
                sbQueryfn.Append(sbQueryfn);
                sbQueryfn.Append("UPDATE Lead SET Description4 ='1',  Status_DuplicateID =  1  ,Process_DuplicateID= 1  where Process_DuplicateID  =  0 ");

                string a111 = sbQueryfn.ToString();
                db.Database.ExecuteSqlCommand(sbQueryfn.ToString());
            }
        }
        public void LeadGW(DataTable sa)
        {
            List<Lead> listLead = new List<Lead>();
            for (int i = 0; i < sa.Rows.Count; i++)
            {
                Lead row = new Lead();
                row.Id= Convert.ToInt64( sa.Rows[i]["Id"].ToString());
                row.Band_CC= sa.Rows[i]["band_cc"].ToString();
                row.Band_UPL= sa.Rows[i]["band_cc"].ToString();
                row.Bound_Code= sa.Rows[i]["bound_code"].ToString();
                row.CC_Limit= Convert.ToDecimal(sa.Rows[i]["cc_limit"].ToString());
                row.Income_Level= Convert.ToDecimal(sa.Rows[i]["income_level"].ToString());
                row.National_Id= sa.Rows[i]["national_id_number"].ToString();
                row.Phone_Number= sa.Rows[i]["phone_number"].ToString();
                row.Post_Date= Convert.ToDateTime( sa.Rows[i]["post_date"].ToString());
                row.Province= sa.Rows[i]["province"].ToString();
                row.Province_Code= sa.Rows[i]["province_code"].ToString();
                row.Score_Range= sa.Rows[i]["score_range"].ToString();
                row.UPL_Interest= Convert.ToDouble( sa.Rows[i]["upl_interest"].ToString());
                row.UPL_Limit= Convert.ToDecimal(sa.Rows[i]["upl_limit"].ToString());
                row.Valid_Date= Convert.ToDateTime(sa.Rows[i]["valid_date"].ToString());
                listLead.Add(row);
            }
        ...
        public DataTable TransferLead(List<Lead_Import> listLeadIP,string duAn, string sanPham, string boundCode, string postDate)
        {
            List<Lead> listLead = new List<Lead>();
            ...
            foreach (var item in listLeadIP)
            {
                ...
                if ( item.San_Pham== "CC")
                {
                    ...
                    {
                        rowLead.Valid_Date = Convert.ToDateTime(item.Scoring_CC).AddDays(60);
                    }
                }
                if (item.San_Pham == "UPL")
                {
                    ...
                    {
                        rowLead.Valid_Date = Convert.ToDateTime(item.Scoring_UPL).AddDays(60);
                    }
                }
                ///////////////////////////////////////////////////////
        ...
        private ImportLeadModel GetLead(ImportLeadModel row)
        {
            ImportLeadModel leadImportFileExcelDto = new ImportLeadModel();
            StringBuilder sbThongBaoLoiTemp = new StringBuilder("");
            string ThongBaoloi = "Không đúng định dạng;";
            leadImportFileExcelDto = new ImportLeadModel();



            if (row.Phone_Number != null && row.Phone_Number != "")
            {
                if (CheckPhone(row.Phone_Number))
                {
                    leadImportFileExcelDto.Phone_Number = row.Phone_Number;
                }
                else
                {
                    sbThongBaoLoiTemp.Append("Phone_Number : " + ThongBaoloi);
                }
            }
            if (sbThongBaoLoiTemp.Length > 1)
            {
                sbThongBaoLoi.Append(row.Phone_Number);
                sbThongBaoLoi.Append(" - ");
                sbThongBaoLoi.Append(sbThongBaoLoiTemp.ToString());
                resultKiemTraLoiDuLieu.Add(row.Phone_Number, sbThongBaoLoi.ToString());
            }



            return leadImportFileExcelDto;
        }

        private bool CheckPhone(string SDT)
        {
            return Regex.Match(SDT, @"^(\d{10})$").Success && SDT[0] == '0';
        }
    }
}
